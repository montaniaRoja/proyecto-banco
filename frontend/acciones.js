function crearCliente() {
    ...
    fetch('http://localhost:3000/api/cliente/create', {
        method: 'POST',
        headers: {
...
function agregarClienteATabla(cliente) {
    ...
    let rowNode = table.row.add([
        ...
        cliente.correo,
        cliente.direccion,
        '<button class="btn btn-primary btn-sm" data-id="' + cliente.id + '" data-nombre="' + cliente.nombre + '" data-nodoc="' + cliente.no_doc + '" data-bs-toggle="modal" data-bs-target="#staticBackdrop02">Crear Cuenta</button>'
    ]).draw().node();
...
document.addEventListener('DOMContentLoaded', function () {
    fetch('http://localhost:3000/api/cliente/list')
    ...
    .then(function (data) {
        ...
        if (Array.isArray(data)) {
            ...
            data.forEach(function (item) {
                ...
                row.insertCell().append(accountButton,' ', transButton);
